...
class utterance_encoding(nn.Module):
    ...
    def forward(self, data):
        ...
        user_utterance = user_utterance.reshape(
            -1, max_utterance_len
        )  # [batch * ?, max_utterance_len]
        user_utterance_mask = user_utterance.ne(
            0
        ).detach()  # [batch * ?, max_utterance_len]

        system_utterance = data["system_utterance"]  # [batch, ?, max_utterance_len]
        system_utterance = system_utterance.reshape(
            -1, max_utterance_len
        )  # [batch * ?, max_utterance_len]
        system_utterance_mask = system_utterance.ne(
            0
        ).detach()  # [batch * ?, max_utterance_len]

        encoded_user_utterance = self.enc(
            user_utterance, attention_mask=user_utterance_mask.float()
        )[
            0
        ]  # [batch * ?, max_utterance_len, hidden_size]
        pooling_user_utterance = universal_sentence_embedding(
            encoded_user_utterance, user_utterance_mask
        )  # [batch * ?, hidden_size]
        pooling_user_utterance /= np.sqrt(
            pooling_user_utterance.size()[-1]
        )  # # [batch * ?, hidden_size]

        encoded_system_utterance = self.enc(
            system_utterance, attention_mask=system_utterance_mask.float()
        )[
            0
        ]  # [batch * ?, max_utterance_len, hidden_size]
        pooling_system_utterance = universal_sentence_embedding(
            encoded_system_utterance, system_utterance_mask
        )  # [batch * ?, hidden_size]
        pooling_system_utterance /= np.sqrt(
            pooling_system_utterance.size()[-1]
        )  # [batch * ?, hidden_size]
        # [batch, ?, hidden_size]
        return pooling_user_utterance.reshape(
            batch_size, conversation_len, -1
        ), pooling_system_utterance.reshape(batch_size, conversation_len, -1)
...
class posterior_conversation_encoding(nn.Module):
    def __init__(self, args=None):
        ...
        self.lstm = nn.LSTM(
            ...
            num_layers=self.args.BiLSTM_layers,
            bidirectional=True,
            batch_first=True,
        )
...
class crf(nn.Module):
    def __init__(self, args=None):
        ...
        def make_param():
            p = nn.Parameter(torch.empty(2, 2))
            nn.init.normal_(p, mean=0.0, std=0.1)
            return p

        self.matrice_all = make_param()

        # role
        self.matrice_u2s = make_param()
        self.matrice_s2u = make_param()

        # the number of times of initiative-taking of the system
        self.matrice_u2s_I0 = make_param()  # no initiave-taking before
        self.matrice_u2s_I1 = make_param()  # 1 system's initiave-taking before
        self.matrice_u2s_I2_more = make_param()  # 2 and more

        # the distance from the last system's initiative-taking to the current system's turn
        self.matrice_u2s_Id2 = make_param()  # consecutive  e.g., 4-2=2
        self.matrice_u2s_Id4_more = make_param()  # inconsecutive e.g., 6-2=4 and  8-2=6

        self.matrice_1_2 = make_param()  # u2s
        self.matrice_2_3 = make_param()  # s2u
        self.matrice_3_4 = make_param()  # u2s
        self.matrice_4_5 = make_param()  # s2u
        self.matrice_5_6 = make_param()  # u2s
        self.matrice_6_7 = make_param()  # s2u
        self.matrice_7_8 = make_param()  # u2s
        self.matrice_8_9 = make_param()  # s2u
        self.matrice_9_10 = make_param()  # u2s
        self.matrice_10_11 = make_param()  # s2u
        self.matrice_11_12 = make_param()  # u2s
        self.matrice_12_13 = make_param()  # s2u
        self.matrice_13_14 = make_param()  # u2s
        self.matrice_14_15 = make_param()  # s2u
        self.matrice_15_16 = make_param()  # u2s
        self.matrice_16_17 = make_param()  # s2u
        self.matrice_17_18 = make_param()  # u2s
        self.matrice_18_19 = make_param()  # s2u
        self.matrice_19_20 = make_param()  # u2s
    ...
    def forward(self, emission_scores, label, prior, posterior_sequence, state):
        ...
        with torch.amp.autocast(device_type="cuda", enabled=False):
            ...
            padding_matrice = torch.zeros(
                2, 2, dtype=torch.float32, device=emission_scores.device
            )

            who2who_subsidiary = torch.stack(
                [self.matrice_u2s, self.matrice_s2u, padding_matrice], 0
            )  # [3, 2, 2]
            Intime_subsidiary = torch.stack(
                [
                    self.matrice_u2s_I0,
                    self.matrice_u2s_I1,
                    self.matrice_u2s_I2_more,
                    padding_matrice,
                ],
                0,
            )  # [4, 2, 2]
            Distance_subsidiary = torch.stack(
                [self.matrice_u2s_Id2, self.matrice_u2s_Id4_more, padding_matrice], 0
            )  # [3, 2, 2]
            ...
            overall_subsidiary = torch.stack(
                [self.matrice_all, padding_matrice], 0
            )  # [2, 2, 2]
            bank = [
                who2who_subsidiary,
                position_subsidiary,
            ...
            if self.args.mode == "train":
                ...
                assert (
                    front_pointers.shape[0]
                    == back_pointers.shape[0]
                    == conversation_len - 1
                )

                sum_emission_score = torch.sum(
                    emission_scores[range(conversation_len), label]
                )
                ...
                for index in range(conversation_len):
                    if index > 0:  # sanity check
                        if self.args.model == "VanillaCRF":
                            ...
                            combined_matrice = (
                                bank[0][state["who2who"][index]]
                                + bank[1][state["position"][index]]
                            )
                        ...
                        sum_transition_score += combined_matrice[
                            front_pointers[index - 1], back_pointers[index - 1]
                        ]  # select right values from the matrice
                ...
                for index in range(conversation_len):
                    ...
                    if index == 0:
                        assert torch.equal(
                            combined_matrice,
                            torch.zeros(
                                2, 2, dtype=torch.int32, device=emission_scores.device
                            ),
                        )

                    alpha = torch.logsumexp(
                        alpha.T
                        + emission_scores[index].unsqueeze(0)
                        + combined_matrice,
                        dim=0,
                        keepdim=True,
                    )  # [1, 2]  row vector
                ...
                backtrace = []
                alpha = torch.full(
                    (1, 2), 0.0, device=emission_scores.device
                )  # [1, 2]  row vector
                ...
                for index in range(conversation_len):
                    ...
                    alpha = (
                        alpha.T + emission_scores[index].unsqueeze(0) + combined_matrice
                    )  # [2, 2]
                ...
                assert torch.equal(
                    backtrace[0],
                    torch.zeros(2, dtype=torch.int32, device=emission_scores.device),
                )
...
class BILSTMCRF(nn.Module):
    def __init__(self, args):
        ...
        self.posterior_conversation_encoding = posterior_conversation_encoding(
            args=args
        )
    ...
    def forward(self, data):
        ...
        for i in range(pair_num):
            ...
            previous_utterance_sequence.append(
                pooling_user_utterance[:, i, :].unsqueeze(1)
            )  # add user's utterance [1, 1, hidden_size]
            prior_utterance_sequence = torch.cat(
                previous_utterance_sequence, 1
            )  # [1, 2i+1, hidden_size]
            ...
            previous_utterance_sequence.append(
                pooling_system_utterance[:, i, :].unsqueeze(1)
            )  # add corresponding system's utterance [1, 1, hidden_size]
            posterior_utterance_sequence = torch.cat(
                previous_utterance_sequence, 1
            )  # [1, 2i+2, hidden_size]
            assert posterior_utterance_sequence.shape[1] == 2 * (i + 1)

            logger["role"].append("system")
            logger["system_I"].append(
                data["system_I_label"][:, i].squeeze().item()
            )  # add 1 or 0

            assert len(logger["role"]) == len(logger["system_I"]) == 2 * (i + 1)

            previous_I_label_sequence.append(
                data["user_I_label"][:, i].unsqueeze(1)
            )  # add user's utterance I label [1, 1]
            previous_I_label_sequence.append(
                data["system_I_label"][:, i].unsqueeze(1)
            )  # add system's utterance I label [1,1]
            I_label_sequence = torch.cat(previous_I_label_sequence, 1)  # [1, 2i+2]
            assert I_label_sequence.shape[1] == 2 * (i + 1)

            I_label_sequence_batch.append(
                I_label_sequence.squeeze().tolist()
            )  # [[2], [4], ...] only used for inference
                ...
                if turn_index == 0:
                    ...
                    if logger["role"][turn_index - 1] == "system":
                        ...
                        if I_times == 0:
                            ...
                            for turn_index_ in range(
                                len(logger["system_I"][0 : turn_index - 1])
                            ):
            ...
            prior_hidden_squence = self.prior_conversation_encoding(
                prior_utterance_sequence
            )
            prior_emission_score = self.prior_e_project(prior_hidden_squence[:, -1, :])

            if self.args.mode == "train":
                # obtain emission score
                posterior_hidden_squence = self.posterior_conversation_encoding(
                    posterior_utterance_sequence
                )
                posterior_emission_scores = self.posterior_e_project(
                    posterior_hidden_squence
                )

                gold_score, total_score = self.crf(
                    posterior_emission_scores.squeeze(0),
                    I_label_sequence.squeeze(0),
                    prior_hidden_squence[:, -1, :].squeeze(0),
                    posterior_hidden_squence.squeeze(0),
                    state,
                )
                ...
                posterior_emission_score_batch.append(
                    posterior_emission_scores[:, -1, :].unsqueeze(1)
                )

            # End of training cycle
            elif self.args.mode == "inference":
                partial_posterior_hidden_squence = self.posterior_conversation_encoding(
                    prior_utterance_sequence
                )  #  [1, 2i+1, 2*hidden_size_BiLSTM]
                partial_posterior_emission_scores = self.posterior_e_project(
                    partial_posterior_hidden_squence
                )  # [1, 2i+1, 2]

                combined_emission_scores = torch.cat(
                    [
                        partial_posterior_emission_scores,
                        prior_emission_score.unsqueeze(1),
                    ],
                    1,
                )  # [1, 2i+2, 2]
                predicted_path = self.crf(
                    combined_emission_scores.squeeze(0),
                    I_label_sequence.squeeze(0),
                    prior_hidden_squence[:, -1, :].squeeze(0),
                    partial_posterior_hidden_squence.squeeze(0),
                    state,
                )
                assert (
                    len(predicted_path)
                    == combined_emission_scores.shape[1]
                    == (partial_posterior_emission_scores.shape[1] + 1)
                    == (partial_posterior_hidden_squence.shape[1] + 1)
                    == (prior_hidden_squence.shape[1] + 1)
                )
                predicted_path_batch.append(predicted_path)  # [[2], [4], ...]
                predicted_path_batch_from_emission.append(
                    combined_emission_scores.squeeze(0).max(1)[1].tolist()
                )  # [1, 2i+2, 2] --> [2i+2, 2] -->[2i+2]

        if self.args.mode == "train":
            gold_score_tensor = torch.cat(gold_score_batch)  # [pair_num]
            total_score_tensor = torch.cat(total_score_batch)  # [pair_num]

            prior_emission_score_tensor = torch.cat(
                prior_emission_score_batch, 1
            ).squeeze(
                0
            )  # [pair_num, 2]
            posterior_emission_score_tensor = torch.cat(
                posterior_emission_score_batch, 1
            ).squeeze(
                0
            )  # [pair_num,2]
            ...
            assert (
                pair_num
                == prior_emission_score_tensor.shape[0]
                == posterior_emission_score_tensor.shape[0]
            )
            assert (
                prior_emission_score_tensor.shape[1]
                == posterior_emission_score_tensor.shape[1]
            )  # 2

            loss_crf = torch.mean(
                total_score_tensor - gold_score_tensor
            )  # average each sample
            loss_mle_e = F.mse_loss(
                prior_emission_score_tensor, posterior_emission_score_tensor.detach()
            )  # [pair_num, 2]
            ...
            assert (
                len(predicted_path_batch[0])
                == len(predicted_path_batch_from_emission[0])
                == len(I_label_sequence_batch[0])
                == 2
            )

            if len(predicted_path_batch) > 1:
                assert (
                    len(predicted_path_batch[1])
                    == len(predicted_path_batch_from_emission[1])
                    == len(I_label_sequence_batch[1])
                    == 4
                )
...
class ContextEncoding(nn.Module):
    ...
    def forward(self, data):
        ...
        encoded_context = self.enc(context, attention_mask=context_mask.float())[
            0
        ]  # [batch * ?, max_context_len, hidden_size]

        return encoded_context[:, 0, :].reshape(
            batch_size, pair_num, -1
...
class OneStep(nn.Module):
    def __init__(self, args=None, actiom_embedding=None):
        ...
        self.args = args
        self.gru = nn.GRU(
            self.args.hidden_size * 2, self.args.hidden_size, bidirectional=False
        )
    ...
    def __init__(self, args, config, mlb):
        ...
        if args.model == "mlc":
            ...
            self.action_embedding = nn.Embedding(
                action_num, self.args.hidden_size, padding_idx=0
            )
        ...
        if self.args.model == "mlc":
            if self.args.task == "SIP-AP":
                if self.args.mode == "inference" and self.args.Oracle_SIP == False:
                    d = self.d_embedding(
                        data["system_I_prediction"]
                    )  # [batch_size, pair_num, 768]
                else:
                    d = self.d_embedding(
                        data["system_I_label"]
                    )  # [batch_size, pair_num, 768]
                logits = self.map_state2action(
                    torch.cat([cls, d], 2)
                )  # [batch=1, pair_num, 2*768]-->[batch_size, pair_num, 23/12]
            elif self.args.task == "AP":
                logits = self.map_state2action(
                    cls
                )  # [batch_size, pair_num, 768]-->[batch_size, pair_num, 23/12]
            else:
                raise NotImplementedError
            ...
            if self.args.mode == "train":
                loss = F.binary_cross_entropy_with_logits(
                    logits.squeeze(0),
                    data["system_action_label"].float().squeeze(0).detach(),
                ).unsqueeze(0)
                return {"loss": loss}
            elif self.args.mode == "inference":
                predicted = torch.where(
                    torch.sigmoid(logits) > 0.5, 1, 0
                )  # [batch=1, pair_num, 23/12]
                ...
                for row in predicted.squeeze(0):
                    predicted_verfied.append(
                        [
                            self.config.action[idx]
            ...
            if self.args.task == "SIP-AP":
                ...
                initialised_decoder_state = (
                    self.map_enc2dec(torch.cat([cls, d], 2)).squeeze(0).unsqueeze(1)
                )
            elif self.args.task == "AP":
                # [batch, pair_num, 768]-->[batch, pair_num, 768] --> [pair_num, 768]--> [pair_num, 1, 768]
                initialised_decoder_state = (
                    self.map_enc2dec(cls).squeeze(0).unsqueeze(1)
                )
            else:
                raise NotImplementedError

            system_action_sequence = data["system_action_sequence"].squeeze(
                0
            )  # [batch_size, pair_num, max_target_length]-->[pair_num, max_target_length]
            pair_num, max_target_length = (
                system_action_sequence.size()
            )  # [pair_num, max_target_length]

            if self.args.mode == "train":
                token_index = (
                    torch.Tensor([1] * pair_num).unsqueeze(1).long().cuda()
                )  # [pair_num, 1]
                ...
                for t in range(max_target_length):
                    output, state = self.dec(
                        token_index, states[-1], initialised_decoder_state
                    )
                    # output [pair_num, 1, hidden_size]
                    # state  [pair_num, 1, hidden_size]
                    output_on_action = self.map_state2action(
                        output
                    )  # [pair_num, 1, label_size]
                    states.append(state)
                    outputs_on_actions.append(output_on_action)
                    token_index = system_action_sequence[:, t].unsqueeze(
                        1
                    )  # [pair_num, 1]

                assert (len(states) - 1) == len(outputs_on_actions) == max_target_length

                outputs_on_actions = torch.cat(
                    outputs_on_actions, dim=1
                )  # [pair_num, max_target_length, label_size]

                loss = F.cross_entropy(
                    outputs_on_actions.view(-1, outputs_on_actions.size(-1)),
                    data["system_action_sequence"].view(-1),
                    ignore_index=0,
                )
                return {"loss": loss}
                ...
                greedy_indices = []
                greedy_ends = (
                    torch.Tensor([0] * pair_num).unsqueeze(1).long().cuda() == 1
                )  # [pair_num, 1] all values are false
